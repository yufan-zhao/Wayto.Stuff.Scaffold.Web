<template>
    <div class="special-rule-epic-danger-sidebar-view">
        <div class="ops">
            <span class="add-btn" @click="onInsertClick(null)">+ 添加危大工程类型</span>
        </div>
        ...
        <SpecialRuleEpicDangerTypeModal
            ref="mainModal"
            :insert="specialEpicDangerService.save.bind(specialEpicDangerService)"
            :update="specialEpicDangerService.updateById.bind(specialEpicDangerService)"
...
import SpecialRuleEpicDangerTypeModal from "./special-rule-epic-danger-type-modal.vue";
...
export default class SpecialRulePositiveNegativeSidebar extends CommonView
{
    ...
    {
        this._mainModal.open("insert", this.specialTypeId, data);
    }

    protected onUpdateClick(data: any)
    {
        this._mainModal.open("update", this.specialTypeId, data);
    }
    ...
    protected onDeleteClick(data: any)
    {
        this._warningModal.open({
            title: "确定删除",
            content: "是否确定删除当前危大工程类型节点",
            onOk: async () => {
                try
                {
                    await this.specialEpicDangerService.virtualRemoveById(data.id);
