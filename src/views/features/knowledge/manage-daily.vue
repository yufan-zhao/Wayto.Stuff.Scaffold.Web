        ...
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            <i-form slot="search-left" ref="searchBox" @submit.native.prevent>
                ...
                <i-form-item label="文章类型：">
                   <i-select  v-model="filters.safetyTrainingType" maxlength="20" @on-change="onSearch" clearable>
...
import { DailyService, DictService , SpecialTypeService, CustomerService } from "src/services";
import { service } from "src/common/decorator";
import { ExtendUtils } from "src/common/utils";
import InsertUpdateModal from "./_components/insert-update-modal-daily.vue";
    ...
    [
        {
            render: (h, {row}: any) =>
            {
                let nameDiv = ExtendUtils.tooltipElement(row.monthBelong + "月", 14, h, "300px", "strong","f18 vb");
                // let tip = ExtendUtils.createTipElement(h, "草稿", "#FF4D3F", false);
                let specialTypeName = ExtendUtils.createTipElement(h, row.type ? "季节性施工安全提示" : "安全日历", "#0DBF47", false);

                return h("div", {class: "col-container"}, [
                    h("div", {class: "content"}, [
                        // 标题部分
                        h("i-row", {class: "title"}, [
                            h("i-col", {attrs: {span: "24"}}, [nameDiv, specialTypeName])
                        ]),
                        // 说明部分
                        h("i-row", {class: "desc"}, [
