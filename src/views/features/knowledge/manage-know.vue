        ...
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            <i-form slot="search-left" ref="searchBox" @submit.native.prevent>
                ...
                <i-form-item label="文章类型：">
                   <i-select  v-model="filters.knowledgeType" maxlength="20" @on-change="onSearch" clearable>
                        <i-option v-for="item in selectListObj.typeList" :key="item.id" :value="item.id">{{item.typeName}}</i-option>
                    </i-select> 
                </i-form-item>

                <i-form-item label="业主单位：" v-if="filters.knowledgeType === 2">
            ...
            <template slot="operation-btn">
                <i-button type="primary" @click="onOperations('insert')" v-permission="permissionCode.KNOWLEDGE_TRAIN_XZ">新增</i-button>
            </template>
            ...
            <template slot="page-table">
                <ux-switch-table 
                    ...
                    :load-func="testPaginTableService.paginKnow.bind(testPaginTableService)"
                    @on-row-click="onRowClick"
                    @no-data="detail = {}"
                >
                    <template slot="detail">
                        <u-detail ref="detail" :detail="detail" :editAuthCode="permissionCode.KNOWLEDGE_TRAIN_BJ" :deleteAuthCode="permissionCode.KNOWLEDGE_TRAIN_SC" @operations="onOperations"></u-detail>
                    </template>
                </ux-switch-table>
            </template>
        </u-page-content>
...
import { SafetyService, DictService , SpecialTypeService, CustomerService } from "src/services";
import { service } from "src/common/decorator";
import { ExtendUtils } from "src/common/utils";
import InsertUpdateModal from "./_components/insert-update-modal-know.vue";
...
export default class ManageView extends CommonView
{
    ...
    protected testPaginTableService: SafetyService;
    ...
    [
        {
            render: (h, {row}: any) =>
            {
                let nameDiv = ExtendUtils.tooltipElement(row.specialName, 14, h, "300px", "strong","f18 vb");
                let tip = ExtendUtils.createTipElement(h, "草稿", "#FF4D3F", false);
                let specialTypeName = ExtendUtils.createTipElement(h, row.specialTypeName, "#0DBF47", false);

                return h("div", {class: "col-container"}, [
                    h("div", {class: "content"}, [
                        // 标题部分
                        h("i-row", {class: "title"}, [
                            h("i-col", {attrs: {span: "24"}}, [nameDiv, specialTypeName,row.release ? "" : tip])
                        ]),
                        // 说明部分
                        h("i-row", {class: "desc"}, [
                            h("i-col", {attrs: {span: "12"}}, `${row.releaseDate || row.createTime || "-"}`),
                            h("i-col", {attrs: {span: "12"}}, row.customerName ? `${row.customerName || "-"}` : "")
                        ])
                    ])
                ]);
            }
    ...
    protected onSearch(): void
    {
        ...
        if(this.filters.knowledgeType !== 2)
        {
    ...
    {
        // let typeList = await this.dictService.getTargetContentList(SYSTEM_CONFIG_TYPE.SAFETY_TRAINING);
        this.selectListObj.typeList = [
            {id: 1, typeName: "标准体系"},
            {id: 2, typeName: "客户体系"}
        ];
