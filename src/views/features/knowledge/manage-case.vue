        ...
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            <i-form slot="search-left" ref="searchBox" @submit.native.prevent>
                ...
                <i-form-item label="文章类型：">
                   <i-select  v-model="filters.excellentCaseType" maxlength="20" @on-change="onSearch" clearable>
            ...
            <template slot="operation-btn">
                <i-button type="primary" @click="onOperations('insert')" v-permission="permissionCode.KNOWLEDGE_CASE_XZ">新增</i-button>
            </template>
            ...
            <template slot="page-table">
                <ux-switch-table 
                    ...
                    :load-func="testPaginTableService.pagincase.bind(testPaginTableService)"
                    @on-row-click="onRowClick"
                    @no-data="detail = {}"
                >
                    <template slot="detail">
                        <u-detail ref="detail" :detail="detail" :editAuthCode="permissionCode.KNOWLEDGE_CASE_BJ" :deleteAuthCode="permissionCode.KNOWLEDGE_CASE_SC" @operations="onOperations"></u-detail>
                    </template>
                </ux-switch-table>
            </template>
        </u-page-content>
...
export default class ManageView extends CommonView
{
    ...
    protected async onOperations(type: string, data?: any)
    {
        switch(type)
        {
            case "insert":
            {
                this._mainInsertUpdateModal.open({knowledgeType: 3});
                break;
            }
            case "update":
            {
