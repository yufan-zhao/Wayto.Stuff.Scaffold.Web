                ...
                <i-row type="flex" justify="start"  class="item-content-1">
                    ...
                    <i-col span="24">
                        <i-form-item prop="projectId" class="customed-form-item" label="项目名称" :label-width="120">
                            <i-select  v-model="formModel.projectId" maxlength="20" :disabled="!formModel.customerId" @on-change="projectItemChange" clearable filterable>
            ...
            <div class="project-modal">
                    ...
                    <i-row type="flex" justify="start">
                        <i-col span="24">
                            <i-form-item prop="consultNum" class="customed-form-item" label="诊治编号" :label-width="120">
                                ...
                                <i-input v-model="formModel.consultNum" placeholder="请输入诊治编号" maxlength="255" style="width:73%">
                                       <span class="dsib mt5" style="" slot="prefix">{{projectCodesNUm}}</span>
                                        <!-- slot="prefix" -->
                                </i-input>
                            </i-form-item>
                        </i-col>
                        ...
                        <i-col span="12">
                            <i-form-item prop="priority" class="customed-form-item" label="诊治优先级" :label-width="120">
                                <i-select  v-model="formModel.priority" maxlength="20">
                                    <i-option v-for="item in [1,2,3]" :key="item" :value="item">{{item}}级</i-option>
                                </i-select> 
                            </i-form-item>
                        </i-col>
                        <i-col span="12">
                        ...
                        <i-col span="12">
                            <i-form-item prop="consultType" class="customed-form-item" label="诊治类型" :label-width="120">
                                <i-select  v-model="formModel.consultType" maxlength="20">
                                    <i-option v-for="item in selectListObj.diagonalList" :key="item.id" :value="item.id">{{item.typeName}}</i-option>
                                </i-select> 
                            </i-form-item>
                        </i-col>
                        ...
                        <i-col span="12">
                            <i-form-item prop="captain" class="customed-form-item" label="诊治组长" :label-width="120">
                                <i-select  v-model="formModel.captain" maxlength="20">
                                    <i-option v-for="item in selectListObj.getLeaders3" :key="item.userId" :value="item.userId">{{item.name}}</i-option>
                                </i-select> 
                            </i-form-item>
                        </i-col>
                        <i-col span="12">
                            <i-form-item prop="crews" class="customed-form-item" label="诊治组员" :label-width="120">
                                <i-select  v-model="formModel.crews" maxlength="20" multiple>
                                    <i-option v-for="item in selectListObj.getLeaders2" :key="item.userId" :value="item.userId">{{item.name}}</i-option>
                                </i-select> 
                            </i-form-item>
                        </i-col>
                    ...
                    <i-row type="flex" justify="start"  class="item-content-1">
                        <i-col span="24">
                            <i-form-item prop="255555" class="customed-form-item" label="" :label-width="0">
                               <i-radio-group v-model="formModel.specialTypeFlags" @on-change="specialTypeFlagChage">
                                    <i-radio :label="1" :value="1" v-if="showSys">
                                        <span>标准体系</span>
                                    </i-radio>
                                    <i-radio :label="0" :value="0"  v-if="!showSys">
                                        <span>客户体系</span>
                                    </i-radio>
                                    <!-- {{formModel.specialTypeFlag}} -->
                                </i-radio-group>
                            </i-form-item>
                        </i-col>
                        <i-col span="24">
                            <i-form-item prop="255555" class="customed-form-item" label="" :label-width="0">
                                <!-- 标准体系 列表 -->
                               <i-radio-group v-model="formModel.specialTypeId" v-if="formModel.specialTypeFlags" @on-change="specialTypeChage">
                                ...
                                <!-- 客户体系 列表 -->
                                <i-radio-group v-model="formModel.specialTypeId" v-if="!formModel.specialTypeFlags" @on-change="specialTypeChage">
                                    <i-radio :label="item.id" :value="item.id" :disabled="false" v-for="item in customerSpecailList" :key="item.id">
                                        <span>{{item.name}}({{item.code}})</span>
                                    </i-radio>
                                </i-radio-group>
                            </i-form-item>
                        </i-col>
                        <i-col span="24" v-if="specialIdsList && specialIdsList.length && formModel.specialTypeFlags">
                            <!-- 标准体系 子列表 -->
                            <i-form-item prop="255555" class="customed-form-item" label="" :label-width="0">
                               <i-checkbox-group v-model="formModel.specialIds" @on-change="specialIdsChage">
                                    <i-checkbox :label="item.id" :value="item.id" :disabled="false" v-for="item in specialIdsList" :key="item.id">
                ...
                <i-form v-show="stepIndex === 2" class="customed-form" ref="form2" :model="formModel" :rules="rules">
                    ...
                    <i-row type="flex" justify="start" v-for="item in (formModel.detail && formModel.detail.buildUnit || [])" :key="'buildUnit' + item.id">
                        ...
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="项目负责人：">{{ item.person ||" - " }}（{{item.phone ||" - "}}）</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="负责人科室：">{{ item.department ||" - " }}</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="安全管理员：">{{ item.securityAdminPerson ||" - " }}（{{item.securityAdminPhone ||" - "}}）</i-form-item>
                        </i-col>
                    </i-row>
                    ...
                    <i-row type="flex" justify="start" v-for="item in (formModel.detail && formModel.detail.construction || [])" :key="'construction' + item.id">
                        ...
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="项目经理：">{{ item.manager ||" - " }}（{{item.managerPhone ||" - "}}）</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="安全总监/经理：">{{ item.charger ||" - " }}（{{item.principalPhone ||" - "}}）</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                    ...
                    <i-row type="flex" justify="start" v-for="item in (formModel.detail && formModel.detail.supervising || [])" :key="'supervising' + item.id">
                        ...
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="项目总监：">{{ item.engineer ||" - " }}（{{item.directorPhone ||" - "}}）</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="安全专监：">{{ item.person ||" - " }}（{{item.phone ||" - "}}）</i-form-item>
                        </i-col>
                        <i-col class-name="u-col" span="12">
                            <i-form-item class="u-form-item" :label-width="120" label="人员数：">{{ item.personNumber ||" - " }}</i-form-item>
                        </i-col>
...
export default class ProjectInsertUpdateModal extends CommonView
{
    ...
    protected projectCodesNUm: string = "";
    ...
    protected rules: any =
    {
        ...
        projectId: [{required: true, message: "请选择项目,项目选择前请选择业主单位"}],
     ...
     * @public
     */
    public async open(data: any, copy?)
    {
        this.stepIndex = 0;
    ...
    public openByDate(consultTime: string)
    {
        this.stepIndex = 0;
        this.title = "新增计划";
        this.onReset({});
        this.$set(this.formModel, "consultTime", consultTime);
        this._curdModal.open();
    }
    ...
    protected onReset(data?)
    {
        ...
        this.formModel =
        {
            ...
            // specialTypeFlags: data.id ? (data.specialTypeFlag ? 1 : 0) : 1,
        ...
        // this.specialTypeFlagChage(data.id ? (data.specialTypeFlag ? 1 : 0) : 1, {}, data.specialTypeId); // 客户/标准体系更改
        this.onchangeCustomer(this.formModel.customerId, data.specialTypeId); // 业主单位更改
     ...
     * 
     */
    // @watch("formModel.customerId", {immediate: true, deep: true})
    protected async onchangeCustomer(value?, specialTypeId?)
    {
        await this.getCTypeList(value, specialTypeId);
        if(value && value !== this.copyId && this.copy === false)
        {
        ...
        await this.projectItemChange(this.formModel.projectId);
        
    }

    protected async getCTypeList(value, specialTypeId?)
    {
        let content = await this.specialTypeService.specialTypes(value);
        this.customerSpecailList = content;
        if(content.length)
        {
            this.$set(this.formModel, "specialTypeFlags", 0);
            // this.formModel.specialTypeFlags = 0;
            this.showSys = false;

            this.specialTypeFlagChage(0,{},specialTypeId);
        }
        else
        {
            // this.formModel.specialTypeFlags = 1;
            this.$set(this.formModel, "specialTypeFlags", 1);
            this.showSys = true;
            this.specialTypeFlagChage(1,{},specialTypeId);
        }
    ...
    protected async specialTypeFlagChage(value,label?, specialTypeId?)
    {
        ...
        if(!value)
        {
            this.formModel.specialTypeId = specialTypeId ? specialTypeId : this.customerSpecailList[0].id;
            
        }
        else
        {
            this.formModel.specialTypeId = specialTypeId ? specialTypeId : this.selectListObj.specialTypesList[0].id;
        }
    ...
    protected async specialTypeChage(value,label?)
    {
        ...
        {
            this.formModel.specialIds = this.formModel.specialIds;
        }else
        {
            this.formModel.specialIds = [];
        }
        // this.formModel.specialIds = [];
    }
    ...
    {
        console.log(value,"----",this.formModel);
        
        let data = JSON.parse(JSON.stringify(this.projectList));
        
        let list = data.filter(item => item.id === value)[0];
        
        this.formModel.detail = list ? list : {
            constructionStageName: "",
            streetCommunity: "",
            projectAddress: "",
            projectTypeName: ""
        ...
        this.$set(this.formModel, "projectId", value);
        
        let projectCode = this.formModel.detail.projectCode;
        let customerNum = this.formModel.detail.customerNum;
        // this.formModel.customerId = list.customerId;
        this.$set(this,"projectCodesNUm",(customerNum || "") + "-");
    }
        ...
        return {
            data: {
                param: Object.assign({}, {
                    ...
                    specialTypeFlag: !!param.specialTypeFlags,
