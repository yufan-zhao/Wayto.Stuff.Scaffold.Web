    ...
    <div class="manage-view">
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            ...
            <template slot="operation-btn">
                <i-button type="primary" @click="onOperations('insert')" v-permission="permissionCode.PLAN_INCOMPLETE_XZ">新增</i-button>
            </template>
            ...
            <template slot="page-table">
                <ux-switch-table 
                    ...
                    :load-func="testPaginTableService.paginIncomplement.bind(testPaginTableService)"
                    @on-row-click="onRowClick"
                    @no-data="detail = {}"
                    :initQueryParams="initQueryParams"
                >
                    <template slot="detail">
                        <u-detail ref="detail" :detail="detail" :createPlanAuthCode="permissionCode.PLAN_INCOMPLETE_CJJH" @operations="onOperations"></u-detail>
                    </template>
                </ux-switch-table>
            </template>
        </u-page-content>
...
export default class ManageView extends CommonView
{
    ...
    [
        {
            render: (h, {row}: any) =>
            {
                let nameDiv = ExtendUtils.tooltipElement(row.projectManager && row.projectManager.projectName || "-", 14, h, "290px", "strong","f18 vb");
                ...
                return h("div", {class: "col-container"}, [
                    h("div", {class: "content"}, [
                        ...
                        h("i-row", {class: "desc"}, [
                            ...
                            h("i-col",
                                {attrs: {span: "24"}},
                                `诊治计划日期：${row.consultTime} ${row.timeMark ? "下午" : "上午"}`)
                        ])
                    ])
                ]);
            }
    ...
    protected async onOperations(type: string, data?: any)
    {
        switch(type)
        {
            ...
            case "begin":
            {
                this.testPaginTableService.startConsult(data.id);
                this.openScorePage(data.id);
                // this.$message.warning("开始诊治弹窗");
                // this._mainInsertUpdateModal.open(JSON.parse(JSON.stringify(data)));
                break;
            }
            default:
