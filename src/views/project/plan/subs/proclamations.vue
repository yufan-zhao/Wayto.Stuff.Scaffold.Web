...
import { PublicityService } from "src/services";
...
export default class InfoView extends View
{
    ...
    [
        {
            ...
            {
                let items = [];
                row && row.commonFiles && row.commonFiles.forEach(item =>
                {
                    let content = h("div",
                    {
                        class: "img-box-item",
    ...
    {
        this.data = await this.publicityService.list({consultPlanId});
    }
}
</script>
