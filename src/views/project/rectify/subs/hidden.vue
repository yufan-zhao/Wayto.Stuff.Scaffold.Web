<template>
    <div class="sub-detail">
       ...
       <section  class="diagnosis-content"  v-if="dataMiddl.length">
        ...
        <section v-if="dataContent.length" >
                <section class="diagnosis-content" v-for="item in dataContent" :key="item.id" >
                        ...
                        <section class="d-f mt20 pr pt5">
                            ...
                            <i-form class="u-form f-1 pl25 bl-ECECEC ml20" label-position="left">
                                <i-row class="d-f">
                                    ...
                                    <div class="f-1">
                                        <i-button type="info" class="recify-btn" v-if="!item.mendInfo || !item.mendInfo.id" @click="onOperations('insert', item)">添加</i-button>
                                        <i-button type="info" class="recify-btn ml10"  v-if="item.mendInfo && item.mendInfo.id" @click="onOperations('update', item)">编辑</i-button>
                                        <i-button type="info" class="recify-btn ml10"  v-if="item.mendInfo && item.mendInfo.id" @click="onOperations('delete', item)">清空</i-button>
                                    </div>
                                </i-row>
                                <i-row type="flex" justify="start">
                                    <i-col class-name="u-col" span="24">
        ...
        <notice-modal
            ref="noticeModal"
            @on-refresh="getPublicity(detail.id,{})"
            :update="mendService.update.bind(mendService)"
            :insert="mendService.insert.bind(mendService)"
        ></notice-modal>
    </div>
...
export default class InfoView extends View
{
    ...
    @service("MendService")
    protected mendService: MendService;
    ...
    protected async getPublicity(consultId,items)
    {
        ...
        this.dataContent = await this.hiddenService.getDangerTree({consultId,...items});
            ...
            case "delete":
            {
                this.$modal.confirm({
                    ...
                    onOk: async () =>
                    {
                        try
                        {
                            await this.mendService.virtualRemoveById(data.mendInfo.id);
                            this.$message.success("操作成功");
                            this.getPublicity(this.detail.id,{});
                            this.getDangerLevel(this.detail.id, this.detail.specialTypeId);
                        } catch (error) {
