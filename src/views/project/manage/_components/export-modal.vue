        ...
        if (type === "submit")
        {
            this._form.validate(async valid =>
            {
                ...
                try
                {
                    ...
                    FileUtils.downloadFile(ApplicationContext.current.settings.mainOriginUrl + ApplicationContext.current.settings.mainPrefix + `/project/projectExport?projectIds=${projectIds.join(",")}`, dayjs().format("YYYY-MM-DD") + "项目");
