<template>
    <div class="sub-detail">
        ...
        <section v-if="data.length">
            <section class="diagnosis-content" v-for="item in data" :key="item.id +'11'" >
                <div class="diagnosis-title"  @click="changeTableShow(item.id)" >
                    ...
                    <p class="diagnosis-info" ref="diagnosis-info-1" >
                        <!-- {{showTableId.filter(val => val === item.id)}} -->
                ...
                <div class="diagnosis-content-info" v-if="showTableId.indexOf(item.id) !== -1">
                    ...
                    <section class="d-f mt20 pr pt5">
                        ...
                        <i-form class="u-form f-1 pl25 bl-ECECEC ml20" label-position="left" v-if="item.mendInfo">
...
export default class InfoView extends View
{
    ...
    protected async getPublicity(projectId,items)
    {
        this.data = await this.hiddenService.getDangerTreeBy({projectId,...items});
    }

    protected async getDangerLevel(projectId,specialTypeId)
    {
        this.dataMiddl = [];
        
        let dataMiddl: any = await this.hiddenService.getDangerLevelBy({projectId,specialTypeId});
