    ...
    <div class="sub-detail positive-negative-report">
        <section class="statistics-info mb20">
            <div class="statistics-item cursor-pointer">
                ...
                <p  class="statistics-name">正负面清单数</p>
            </div>
            <div class="statistics-item cursor-pointer">
                <p class="statistics-num">
                    {{statData.afterRectification}}
                </p>
            ...
            <section class="diagnosis-content" v-for="(item, i) in data" :key="i">
                <div class="diagnosis-title" @click="changeTableShow(item.id)">
                    <p class="title">
                        ...
                        <span class="problem-status" :style="{background: mendStatusColor[item.status]}" v-if="item.type">{{mendStatus[item.status]}}</span>
                    </p>
                ...
                <div class="diagnosis-content-info"  v-if="showTableId.indexOf(item.id) !== -1">
                    ...
                    <section class="d-f pr pt5">
                        ...
                        <i-form class="u-form f-1" label-position="left" v-if="item.type">
                        ...
                        <i-form class="u-form f-1" label-position="left" v-else>
                            <i-row> <h5>问题信息</h5></i-row>
                            <i-row type="flex" justify="start">
                                <i-col class-name="u-col" span="24">
                                    <i-form-item class="u-form-item" label="加分项">{{item.item.details}}</i-form-item>
                                </i-col>
                                <i-col class-name="u-col" span="24">
                                    <i-form-item class="u-form-item" label="加分">{{item.score}}</i-form-item>
                                </i-col>
                                <i-col class-name="u-row" span="24" v-if="Array.isArray(item.commonFileList) && item.commonFileList.length > 0">
                                    <i-form-item class="u-form-item" label="问题图片" v-previewer>
                                        <div class="d-f f-w">
                                ...
                                <i-col class-name="u-col" span="24">
                                    <i-form-item class="u-form-item" label="加分标准">{{item.item.standard}}</i-form-item>
                                </i-col>
                                <i-col class-name="u-col" span="24">
                                    <i-form-item class="u-form-item" label="加分单位">{{item.supervisingUnitNames}}</i-form-item>
                                </i-col>
                                <!-- <i-col class-name="u-col" span="24">
                                    <i-form-item class="u-form-item" label="整改期限">{{item.rectifyDay}}天</i-form-item>
                                </i-col> -->
                        ...
                        <!-- v-if="item.mendInfo" -->
                        <i-form class="u-form f-1 pl25 bl-ECECEC ml20" label-position="left" v-if="item.type">
                            <i-row class="d-f">
