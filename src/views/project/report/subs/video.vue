                ...
                <div class="data-img">
                    ...
                    <div class="img mb5" v-for="val in item.pictureVideo" :key="val.id">
                        <div class="img-div" :style="{'background': `url(${imagePath(val.filePath)}) no-repeat center center`,backgroundSize: 'cover'}"  v-if="inputType.indexOf(val.fileSuffix) !== -1" >
                            ...
                            <img :src="imagePath(val.filePath)"  v-previewer/>
                        </div>
...
export default class InfoView extends View
{
    ...
    {
        let { content } = await this.consultPlanService.getConsultFileListById({consultId: this.detail.id, ...this.filters});
    ...
    protected delImg(id)
    {
        this.$modal.confirm({
            ...
            onOk: async () =>
            {
                try
                {
                    await this.consultPlanService.fileRemoveById(id);
