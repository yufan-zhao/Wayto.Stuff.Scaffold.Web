        ...
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            <i-form slot="search-left" ref="searchBox" @submit.native.prevent>
                <i-form-item label="关键字：">
                    <i-input placeholder="业主单位/对接人电话" v-model="filters.customerDeptInfo" @on-clear="onSearch" @keyup.native.enter="onSearch" clearable></i-input>
                </i-form-item>
            ...
            <template slot="operation-btn">
                <i-button type="primary" @click="onOperations('insert')" v-permission="permissionCode.USER_OWNER_XZ">新增</i-button>
            </template>
...
import { CustomerService, DictService } from "src/services";
...
export default class ManageView extends CommonView
{
    ...
    protected testPaginTableService: CustomerService;
    ...
    [
        {
            render: (h, {row}: any) =>
            {
                let nameDiv = ExtendUtils.tooltipElement(row.deptName, 14, h, "300px", "strong","f18 vb");
                ...
                return h("div", {class: "col-container"}, [
                    h("div", {class: "content"}, [
                        ...
                        h("i-row", {class: "desc d-f"}, [
                            h("i-col", {attrs: {span: "12"}}, `对接人：${row.responsible || "-"}`),
                            h("i-col",
                                {attrs: {span: "12"}},
                                `${row.mobilePhone || ""}`)
                        ])
    ...
    protected async selectList()
    {
        ...
        let contractList = await this.dictService.getTargetContentList(SYSTEM_CONFIG_TYPE.CONTRACT_TYPE);
        this.selectListObj.contractList = contractList; // 合同类型列表
