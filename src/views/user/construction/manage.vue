<template>
    <div class="manage-view">
        <u-page-content :class="{open: isOpenOperation}" :isOpenOperation="isOpenOperation">
            <i-form slot="search-left" ref="searchBox" @submit.native.prevent>
                <i-form-item label="关键字：">
                    <i-input placeholder="单位名称/项目经理" v-model="filters.constructionDeptInfo" @on-clear="onSearch" @keyup.native.enter="onSearch" clearable></i-input>
                </i-form-item>
            ...
            <template slot="operation-btn">
                <i-button type="primary" @click="onOperations('insert')" v-permission="permissionCode.USER_CONSTRUCTION_XZ">新增</i-button>
            </template>
            ...
            <template slot="page-table">
                <ux-switch-table 
                    ...
                    :load-func="constructService.pagin.bind(constructService)"
        ...
        <main-insert-update-modal
            ...
            :insert="constructService.save.bind(constructService)"
            :update="constructService.updateById.bind(constructService)"
            @on-refresh="onSearch"
        >
        </main-insert-update-modal>
    </div>
...
import { ConstructService } from "src/services";
    ...
    [
        {
            render: (h, {row}: any) =>
            {
                let nameDiv = ExtendUtils.tooltipElement(row.constructionName, 14, h, "300px", "strong","f18 vb");
                ...
                return h("div", {class: "col-container"}, [
                    h("div", {class: "content"}, [
                        ...
                        h("i-row", {class: "desc d-f"}, [
                            h("i-col", {attrs: {span: "12"}}, `项目经理：${row.manager || "-"}`),
                            h("i-col",
                                {attrs: {span: "12"}},
                                `${row.managerPhone || ""}`)
                        ])
            ...
            case "delete":
            {
                this.$modal.confirm({
                    ...
                    onOk: async () =>
                    {
                        try
                        {
                            await this.constructService.virtualRemoveById(data.id);
            ...
            case "reset":
            {
                this.$modal.confirm({
                    ...
                    onOk: async () =>
                    {
                        try
                        {
                            await this.constructService.resetPsw({data: {id: data.userId,password: 123456}});
                            this.$message.success("操作成功");
                            this.onSearch();
                        } catch (error) {
