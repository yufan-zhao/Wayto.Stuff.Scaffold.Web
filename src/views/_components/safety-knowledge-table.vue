 ...
 */
@component
export default class SafetyKnowledgeTableView extends View
{
        ...
        if (type === "check-cancel")
        {
            this._warningModal.open({
                ...
                onOk: async () => {
                    try
                    {
                        await this.safetyExportService.saveOrUpdateExportStatus(this.consultId, data.id, false, 0);
        ...
        else if (type === "check")
        {
            this._warningModal.open({
                ...
                onOk: async () => {
                    try
                    {
                        await this.safetyExportService.saveOrUpdateExportStatus(this.consultId, data.id, true, 0);
                        this.$message.success("操作成功");
                        this.initData();
                    }
                    catch(err)
    ...
    {
        this.data.splice(0);
        const res = await this.safetyService.listDetails({consultId: this.consultId});
